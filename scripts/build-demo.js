...
const BEATS = [
  ...
  { frame: '00b-problem.png', text: 'Here is the problem. People do not ask precisely. And the cost is hidden: wrong work that gets redone, endless clarifying ping-backs, context lost to new joiners, and decisions that stall. We call it the ambiguity tax.' },
  ...
  { frame: '11-cta.png', text: 'It is open source and zero dependency. Clone the repo, run npm start, and open localhost port four three two one.' },
  { frame: '08-end.png', text: 'Ambigua. The environment is the disambiguation engine. Live model, live data. Built for the Best Use of Ambiguous AI track — and open on GitHub.' }
];
...
async function main() {
  ...
  // 1. title/problem/cta/end cards are (re)generated by ensureCards() before assembly.
...
function cardHTML({ kicker, title, body = [], accent, titleSize = 84 }) {
  return `<!doctype html><meta charset=utf-8><body style="margin:0;width:${W}px;height:${H}px;background:radial-gradient(1200px 600px at 20% -10%,rgba(124,92,255,.28),transparent 60%),#0a0e17;color:#e7ecf6;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;display:flex;flex-direction:column;justify-content:center;padding:0 120px">
    <div style="font-size:20px;letter-spacing:3px;color:${accent};text-transform:uppercase">${kicker}</div>
    <div style="font-size:${titleSize}px;font-weight:800;margin-top:18px;line-height:1.1">${title}</div>
    ${body.map((t) => `<div style="font-size:30px;color:#8b98b0;margin-top:14px">${t}</div>`).join('')}
...
function ensureCards() {
  const render = (name, html) => {
    const htmlPath = path.join(REEL, name.replace(/\.png$/, '.html'));
    fs.writeFileSync(htmlPath, html);
    execFileSync(CHROME, ['--headless=new', '--disable-gpu', '--hide-scrollbars', `--window-size=${W},${H}`,
      '--default-background-color=0a0e17ff', `--screenshot=${path.join(REEL, name)}`, 'file://' + htmlPath], { stdio: 'ignore' });
    log('· card', name);
  };
  render('00-title.png', cardHTML({ kicker: 'Best Use of Ambiguous AI', title: 'Ambigua', accent: '#c4b5fd' }));
  render('00b-problem.png', cardHTML({ kicker: 'The problem', title: 'The ambiguity tax', accent: '#fbbf24', body: ['Wrong work that gets redone', 'Endless clarifying ping-backs', 'Context lost to new joiners', 'Decisions that stall'] }));
  render('11-cta.png', cardHTML({ kicker: 'Try it', title: 'npm start', accent: '#7dd3fc', body: ['→ http://localhost:4321', 'Zero dependencies · Node 18+', 'github.com/ArthurzWong/ambigua'] }));
  render('08-end.png', cardHTML({ kicker: 'Ambigua — resolve ambiguous asks', title: 'github.com/ArthurzWong/ambigua', accent: '#7dd3fc', titleSize: 52 }));
}
